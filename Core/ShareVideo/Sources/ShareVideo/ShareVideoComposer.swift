...
public enum ShareVideoError: LocalizedError, Equatable {
    /// The recitation (ayah or dhikr) is not cached and could not be downloaded.
    ...
    public var errorDescription: String? {
        switch self {
        case .audioUnavailable:
            String(localized: "Connect once to download this recitation")
...
/// Assembles a 9:16 MP4 from a share card plus a recitation, with a gold
/// equaliser under the card that moves with the reciter's voice.
///
/// Content-agnostic on purpose: it takes a rendered card image and an audio
/// file, so both the Quran reader (an ayah) and Athkar (a dhikr) use it. It
/// therefore lives in `Core/` — feature modules never import each other.
///
...
public enum ShareVideoComposer {
    ...
    /// Builds the MP4 and returns its URL. Throws `ShareVideoError`.
    public static func makeVideo(
        ...
        else { throw ShareVideoError.exportFailed }
        do {
        ...
        guard let job = ExportJob(composition: composition) else { throw ShareVideoError.exportFailed }
        ...
        guard FileManager.default.fileExists(atPath: finalURL.path) else { throw ShareVideoError.exportFailed }
        return finalURL
    }
    ...
    private static func prepareOutputDirectory() throws -> URL {
        ...
        do {
            ...
            throw ShareVideoError.videoWriteFailed
        }
    ...
    static func loudnessEnvelope(asset: AVAsset, track: AVAssetTrack, seconds: Double, fps: Int32) async throws -> [Float] {
        ...
        guard let reader = try? AVAssetReader(asset: asset) else { throw ShareVideoError.audioUnreadable }
        ...
        guard reader.canAdd(output) else { throw ShareVideoError.audioUnreadable }
        reader.add(output)
        guard reader.startReading() else { throw ShareVideoError.audioUnreadable }
        ...
        if reader.status == .failed { throw ShareVideoError.audioUnreadable }
    ...
    static func writeVideo(size: CGSize, seconds: Double, to url: URL,
        ...
        else { throw ShareVideoError.videoWriteFailed }
        try await job.run()
    }
}
    ...
    func export(to finalURL: URL) async throws {
        if #available(iOS 18, macOS 15, *) {
            do {
                ...
                throw ShareVideoError.cancelled
            } catch {
                throw ShareVideoError.exportFailed
            }
        } else {
            ...
            case .cancelled: throw ShareVideoError.cancelled
            default: throw ShareVideoError.exportFailed
            }
        }
    }
...
private actor FrameWriter {
    ...
    init?(url: URL, size: CGSize, frameCount: Int, frameAt: @escaping (Int) -> CVPixelBuffer?) {
        ...
        let settings: [String: Any] = [
            ...
            AVVideoCompressionPropertiesKey: [
                ...
                AVVideoMaxKeyFrameIntervalKey: ShareVideoComposer.framesPerSecond * 2
            ]
        ]
        let input = AVAssetWriterInput(mediaType: .video, outputSettings: settings)
        input.expectsMediaDataInRealTime = false
        self.queue = DispatchSerialQueue(label: "com.engagendy.noor.share-video")
        self.writer = writer
        self.input = input
    ...
    func run() async throws {
        guard writer.canAdd(input) else { throw ShareVideoError.videoWriteFailed }
        writer.add(input)
        guard writer.startWriting() else { throw ShareVideoError.videoWriteFailed }
        ...
        guard writer.status == .completed else { throw ShareVideoError.videoWriteFailed }
    }
    ...
    private func pump() {
        ...
        while input.isReadyForMoreMediaData, index < frameCount {
            ...
            let time = CMTime(value: CMTimeValue(index), timescale: ShareVideoComposer.framesPerSecond)
    ...
    private func abort(with error: ShareVideoError) {
        finished = true
        input.markAsFinished()
