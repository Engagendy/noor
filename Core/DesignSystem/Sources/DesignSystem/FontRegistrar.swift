...
public enum FontRegistrar {
    ...
    /// Registers all five interface families (Settings → App font). All of
    /// them, not just the selected one, because the Settings picker draws
    /// every row in its own family. The files are memory-mapped, so this is
    ...
    public static func registerInterfaceFonts() {
        ...
        let urls = NoorAppFont.allCases
            .flatMap(\.fileNames)
            .compactMap {
                Bundle.module.url(forResource: $0, withExtension: "ttf", subdirectory: "UIFonts")
            }
        assert(urls.count == NoorAppFont.allCases.flatMap(\.fileNames).count,
               "Missing bundled interface font file(s)")
        for url in urls {
            CTFontManagerRegisterFontsForURL(url as CFURL, .process, nil)
        }
    }
